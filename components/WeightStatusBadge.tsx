...
import { MUTED, POSITIVE, AMBER, NEGATIVE, RADIUS_TAG } from "@/lib/theme";
...
const STATUS_STYLE: Record<WeightStatus, { color: string; bg: string; label: string }> = {
  "No target":        { color: MUTED, bg: "rgba(15,30,53,0.06)", label: "" },
  Underweight:        { color: MUTED, bg: "rgba(15,30,53,0.06)", label: "Below target" },
  "In range":         { color: POSITIVE, bg: "rgba(26,74,46,0.08)", label: "On target" },
  Watch:              { color: AMBER, bg: "rgba(122,69,32,0.10)", label: "Slightly above target" },
  "Review for trim":  { color: NEGATIVE, bg: "rgba(139,37,48,0.10)", label: "Above target" },
};
...
export default function WeightStatusBadge({
  ...
  return (
    <span
      className={`inline-block whitespace-nowrap font-mono ${textSize} font-semibold uppercase tracking-[0.12em]`}
      style={{ color, background: bg, padding, borderRadius: RADIUS_TAG }}
    >
      {label}
    </span>
  );
